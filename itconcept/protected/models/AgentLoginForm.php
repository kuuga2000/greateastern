...
class AgentLoginForm extends CFormModel {
	...
	public function checkSuspended($attribute,$params){
		$model = GEAgent::model()->findByAttributes(array('username'=>$this->username));
		if(!empty($model)){
			if($model->is_active==0){
				$this -> addError('password', 'Your account has been suspended, please contact your company!');
			}
		}
	}
	...
	public function authenticate($attribute, $params) {
		...
		if (!$this -> hasErrors()) {
			 ...
			 */
			$this -> _identity = new AgentLogin($this -> username, $this -> password);
	...
	public function login() {
		...
		if ($this -> _identity -> errorCode === AgentLogin::ERROR_NONE) {

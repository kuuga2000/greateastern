<div class="block push-bit">
	...
	<!-- END Login Form -->
	<!-- Reminder Form -->
	<form action="<?php echo $this->baseUrlAgent(TRUE);?>/login/resetpassword" method="post" id="form-reminder" class="form-horizontal form-bordered form-control-borderless display-none">
		<div class="form-group">
		...
		$(".login-button").click(function() {
			...
			$.post($("#form-login").attr("action"),$("#form-login").serialize(),function(response){
				if(response.success==true){
					...
					if(response.AgentLoginForm_username){
						$(".username").addClass("has-error");
						$(".errorUsername").html(response.AgentLoginForm_username).show();
					}else{
						$(".username").removeClass("has-error");
						$(".errorUsername").hide();
					}
					if(response.AgentLoginForm_password){
						$(".password").addClass("has-error");
						$(".errorPassword").html(response.AgentLoginForm_password).show();
					}else{
